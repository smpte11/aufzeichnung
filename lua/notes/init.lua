...
function M._run_database_migrations(db)
    ...
    if not column_exists(db, "task_events", "content_hash") then
        notify("🔄 Running database migration: Adding content_hash column", "info", "database_operations")

        -- Add the content_hash column (nullable) and create its index for faster lookups
        local success, err = pcall(function()
        ...
        if success then
            notify("✅ Database migration completed: content_hash column and index added", "info", "database_operations")
        else
            notify(string.format("❌ Database migration failed: %s", tostring(err)), "error", "database_operations")
        end
    else
        -- Migration 2.5: Fix any records with NULL content_hash (defensive repair)
        local records_to_fix = db:eval([[
            SELECT event_id, task_text
            FROM task_events
            WHERE content_hash IS NULL OR content_hash = ''
        ]])
...
function M._get_task_database(track_type)
    ...
    if not M.sqlite then
        notify("sqlite.lua not available, cannot access task database", "error", "database_operations")
        return nil
    end
...
function M._track_tasks_on_save(bufnr)
    ...
    for _, task in ipairs(tasks) do
        ...
        if not existing or #existing == 0 or existing[1].count == 0 then
            ...
            -- Get last event details - use named column access for safety
            ...
            if config.advanced and config.advanced.debug_mode then
                print(string.format("  RAW DB: state=%s, parent_id=%s, content_hash=%s, task_text=%s",
                    tostring(existing[1].state),
                    tostring(existing[1].parent_id),
...
function M._get_picker_parent_tasks(track_type)
    ...
    for _, row in ipairs(results) do
        ...
        local state_emoji = row.state == "IN_PROGRESS" and utils.icons.rocket or utils.icons.pencil
...
function M._create_dashboard_buffer(track_type, lines)
    ...
    map('y', string.format(':lua require("notes").previous_day_dashboard("%s")<CR>:close<CR>', track_type),
    ...
    map('f', string.format(':lua require("notes").full_review_dashboard("%s")<CR>:close<CR>', track_type),
    ...
    if config.tracking then
        for track_type, track_config in pairs(config.tracking) do
            if track_config.enabled and track_config.filename_patterns then
                table.insert(lines,
                    string.format("  %s: %s", track_type, table.concat(track_config.filename_patterns, ", ")))
            end
        end
    end
