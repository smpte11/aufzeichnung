...
describe("notes.utils", function()
    ...
    describe("add_state_emoji", function()
        it("should add icon to FINISHED state", function()
            local result = utils.add_state_emoji("FINISHED")
            assert.is_not_nil(result:match(""))
        end)

        it("should add icon to IN_PROGRESS state", function()
            local result = utils.add_state_emoji("IN_PROGRESS")
            assert.is_not_nil(result:match(""))
        end)

        it("should add icon to CREATED state", function()
            local result = utils.add_state_emoji("CREATED")
            assert.is_not_nil(result:match(""))
        end)

        it("should handle unknown states (file icon fallback)", function()
            local result = utils.add_state_emoji("UNKNOWN")
            assert.is_not_nil(result:match(""))
        end)

        it("should handle nil state (file icon fallback)", function()
            local result = utils.add_state_emoji(nil)
            assert.is_not_nil(result:match(""))
        end)

        it("should be case insensitive (FINISHED icon)", function()
            local result = utils.add_state_emoji("finished")
            assert.is_not_nil(result:match(""))
        end)
    end)
