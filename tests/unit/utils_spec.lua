...
describe("notes.utils", function()
	...
	describe("add_state_emoji", function()
		it("should add emoji to FINISHED state", function()
			local result = utils.add_state_emoji("FINISHED")
			assert.is_true(result:match("✅"))
		end)

		it("should add emoji to IN_PROGRESS state", function()
			local result = utils.add_state_emoji("IN_PROGRESS")
			assert.is_true(result:match("🚀"))
		end)

		it("should add emoji to CREATED state", function()
	...
	describe("truncate_text", function()
		...
		it("should use custom suffix", function()
			...
			assert.are.equal("Long text>>", result)
		end)
	...
	describe("generate_uuid_v7", function()
		it("should generate valid UUID format", function()
			...
			assert.is_true(uuid:match("^%x%x%x%x%x%x%x%x%-%x%x%x%x%-7%x%x%x%-%x%x%x%x%-%x%x%x%x%x%x%x%x%x%x%x%x$") ~= nil)
		end)
		...
		it("should have correct length", function()
			local uuid = utils.generate_uuid_v7()
			assert.are.equal(36, #uuid) -- 32 hex chars + 4 hyphens
		end)
	end)
